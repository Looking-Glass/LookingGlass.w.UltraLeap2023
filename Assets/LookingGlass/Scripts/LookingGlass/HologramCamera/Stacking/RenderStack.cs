...
namespace LookingGlass {
    ...
    public class RenderStack : IEnumerable<RenderStep> {
        ...
        private bool SetupQuiltIfNeeded(HologramCamera hologramCamera) {
            ...
            if (quiltMix == null || quiltMix.width != renderSettings.quiltWidth || quiltMix.height != renderSettings.quiltHeight) {
                if (quiltMix != null)
                    quiltMix.Release();
                quiltMix = new RenderTexture(renderSettings.quiltWidth, renderSettings.quiltHeight, 0);
                quiltMix.name = "Quilt Mix (" + renderSettings.quiltWidth + "x" + renderSettings.quiltHeight + ")";
        ...
        private void RenderStep(RenderStep step, HologramCamera hologramCamera, RenderTexture mix) {
            HologramRenderSettings hologramCameraRenderSettings = hologramCamera.RenderSettings;
            ...
            switch (step.RenderType) {
                ...
                case LookingGlass.RenderStep.Type.Quilt:
                    ...
                    if (quilt != null) {
                        ...
                        int minViews = Mathf.Min(hologramCameraRenderSettings.numViews, step.RenderSettings.numViews);
                        for (int v = 0; v < minViews; v++)
                            MultiViewRendering.CopyViewBetweenQuilts(step.RenderSettings, v, temp, hologramCameraRenderSettings, v, mix);
