...
namespace LookingGlass {
    public partial class HologramCamera {
        public const float DefaultManualViewCone = 53.512f;
        ...
        [SerializeField] internal float nearClipFactor = DeviceSettings.Get(DeviceType.Portrait).nearClip;

        [Range(0.01f, 40)]
        [Tooltip("Scales the farClipPlane away from the focal plane." +
            "\n\nThe relationship can be mathematically written as the following:" +
            "\nfarClipPlane = max(0.01, focalPlane + size * farClipFactor * depthiness)")]
        [SerializeField] internal float farClipFactor = 4;
        ...
        #region Events
        [Tooltip("A callback that gets called after the calibration has been loaded and the screen size has been set.")]
        [SerializeField] internal LoadEvent onCoreLoaded;
        ...
        #region Optimization
        [SerializeField] internal ViewInterpolationType viewInterpolation = ViewInterpolationType.None;
