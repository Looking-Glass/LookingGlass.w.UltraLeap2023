    ...
    public class MultiViewRenderer : MonoBehaviour {
        ...
        private void LateUpdate() {
            if (RenderPipelineUtil.IsHDRP || RenderPipelineUtil.IsURP) {
                ...
                //TODO: Update finalRawImage material and invoke onAfterScreenBlit... must test with URP / HDRP
            }
        }
        ...
        private void OnRenderImage(RenderTexture source, RenderTexture destination) {
            ...
            if (onAfterScreenBlit != null) {
                InvokeScreenBlitEventAndCopy(quiltMix, destination);
            } else {
                Graphics.Blit(quiltMix, destination, hologramCamera.LightfieldMaterial);
            }
        }
        #endregion
        ...
        private void InvokeScreenBlitEventAndCopy(RenderTexture quiltMix, RenderTexture destination) {
            RenderTexture screenTexture = RenderTexture.GetTemporary(Screen.width, Screen.height, 0, RenderTextureFormat.ARGB32);
            try {
                Graphics.Blit(quiltMix, screenTexture, hologramCamera.LightfieldMaterial);
