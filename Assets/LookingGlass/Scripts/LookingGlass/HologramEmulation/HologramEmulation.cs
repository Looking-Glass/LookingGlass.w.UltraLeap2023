...
namespace LookingGlass {
    ...
    public class HologramEmulation : IDisposable {
        ...
        private CoroutineRunner coroutines;
        ...
        public int ViewCount => renderSettings.numViews;
        ...
        public HologramEmulation(HologramCamera source) {
            ...
            Initialize(source.RenderStack.QuiltMix, source.RenderSettings);
            source.RenderStack.onQuiltChanged += UpdateQuilt;
            source.RenderStack.onRendered += UpdateAfterRenderStack;
            source.onRenderSettingsChanged += UpdateRenderSettings;
        }

        public HologramEmulation(RenderTexture sourceQuilt, HologramRenderSettings RenderSettings) {
            Initialize(sourceQuilt, RenderSettings);
        }

        private void Initialize(RenderTexture sourceQuilt, HologramRenderSettings RenderSettings) {
        ...
        public void Dispose() {
            ...
            if (hologramCamera != null) {
                ...
                hologramCamera.onRenderSettingsChanged -= UpdateRenderSettings;
            }
        ...
        private void CreateHologramTexture() {
            ...
            singleViewTexture = new RenderTexture(renderSettings.ViewWidth, renderSettings.ViewHeight, 0, RenderTextureFormat.Default) {
        ...
        private void SetupPreviewRenderObjects() {
            ...
            coroutines = previewParent.gameObject.AddComponent<CoroutineRunner>();
            coroutines.onUpdate = OnUpdate;

            previewParent.position = new Vector3(100, 0, 0);
        }

        public void SetView(int viewIndex) {
            if (viewIndex < 0 || viewIndex >= renderSettings.numViews)
                throw new ArgumentOutOfRangeException(nameof(viewIndex), viewIndex, nameof(viewIndex) + " must be in range [0, " + renderSettings.numViews + ").");
        ...
        private bool Render(bool force = false) {
            ...
            MultiViewRendering.CopyViewFromQuilt(renderSettings, viewIndex, sourceQuilt, singleViewTexture);
        ...
        private void UpdateObjectOnAnyQuiltChanges() {
            ...
            float aspect = renderSettings.GetAspectOrDefault();
            previewTexture = new RenderTexture((int) (renderSettings.ViewHeight * aspect), renderSettings.ViewHeight, 0);
        ...
        public string GetQuiltDescription() =>
            renderSettings.quiltWidth + "×" + renderSettings.quiltHeight + ", " +
            renderSettings.viewColumns + "×" + renderSettings.viewRows + ", " +
            renderSettings.GetAspectOrDefault() + " aspect";
