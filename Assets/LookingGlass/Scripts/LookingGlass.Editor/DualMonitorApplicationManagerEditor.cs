...
namespace LookingGlass.Editor.DualMonitorApplication {
    ...
    public class DualMonitorApplicationManagerEditor : UnityEditor.Editor {
        ...
        public static void BuildProScenesInteractive(bool buildAndRun) {
            ...
            if (!buildAndRun || buildFolderPath == Path.GetFullPath(".") || (buildAndRun && !Directory.Exists(buildFolderPath))) {
                ...
                buildFolderPath = EditorUtility.SaveFolderPanel("Choose Build Directory", openFolderPath, "");
                if (string.IsNullOrEmpty(buildFolderPath))
                    return;
            }

            bool isFolderEmpty = Directory.GetDirectories(buildFolderPath).Length <= 0 && Directory.GetFiles(buildFolderPath).Length <= 0;
            if (!isFolderEmpty) {
                ...
                try {
                    ...
                    Debug.Log(e.Message);
                    Debug.LogWarning("[LookingGlass] Couldn't overwrite existing build. Make sure build is not running and try again");
                }
            }

            IEnumerable<string> scenesFromBuildSettings = EditorBuildSettings.scenes.Where(s => s.enabled).Select(s => s.path);
            string buildFilePath = buildFolderPath + "/" + Application.productName + ".exe";

            CustomBuildResults results = BuildProScenes(buildAndRun, buildFilePath, scenesFromBuildSettings);
            if (results.success) {
                ...
                if (!buildAndRun) {
                    ...
                    ProcessStartInfo processStartInfo = new ProcessStartInfo(results.extendedUIBuildReport.summary.outputPath);
                    Process.Start(processStartInfo);
                }
            }
        }

        public static CustomBuildResults BuildProScenes(bool buildAndRun, string buildFilePath, IEnumerable<string> scenes) {
            if (PlayerSettings.runInBackground == false) {
                PlayerSettings.runInBackground = true;
                Debug.Log("NOTE: Set " + nameof(PlayerSettings) + "." + nameof(PlayerSettings.runInBackground) + " to true.");
            }
            ...
            if (sceneNamesExtendedUI.Count == 0) {
                Debug.LogError("No \"" + DualMonitorApplicationManager.extendedUIString + "\" scenes found! Please add the extendedUI scenes to build settings (File -> Build Settings).");
                return results;
            }
