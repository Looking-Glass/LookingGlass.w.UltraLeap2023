    ...
    public class BlockUploaderEditor : UnityEditor.Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            try {
                ...
                using (new EditorGUI.DisabledScope(!LookingGlassUserSystem.IsLoggedIn || IsUploading)) {
                    SerializedProperty current = serializedObject.GetIterator();
                    current.NextVisible(true);
                    while (current.NextVisible(false)) {
            ...
            GUILayout.Space(10);
            if (LookingGlassUserSystem.IsLoggedIn) {
                FontStyle prevStyle = GUI.skin.label.fontStyle;
                GUI.skin.label.fontStyle = FontStyle.Italic;
                try {
                    GUILayout.Label("Connected as " + LookingGlassUserSystem.Username + " (" + LookingGlassUserSystem.DisplayName + ")");
                } finally {
                    ...
                    try {
                        ...
                        if (hologram.isPublished) {
                            if (GUILayout.Button("  View Block  ", GUILayout.ExpandWidth(false))) {
                                Application.OpenURL(LookingGlassWebRequests.GetViewURL(LookingGlassUserSystem.Username, hologram.id));
                            }
                        } else {
                            if (GUILayout.Button("  Edit Block  ", GUILayout.ExpandWidth(false))) {
                                Application.OpenURL(LookingGlassWebRequests.GetEditURL(LookingGlassUserSystem.Username, hologram.id));
                            }
                        }
                    } finally {
                        GUI.color = prevColor;
        ...
        private void LoggedOutGUI() {
            ...
            try {
                ...
                if (GUILayout.Button("    Connect to Blocks    ", GUILayout.ExpandWidth(false))) {
                    logInTask = LookingGlassUserSystem.LogIn((object response) => {
                        switch (response) {
            ...
            try {
                using (new EditorGUI.DisabledScope(IsUploading)) {
                    if (GUILayout.Button("    Upload    ")) {
                        ...
                        uploadProgress = LookingGlassUserSystem.UploadFileToBlocks(uploader.QuiltFilePath, uploader.BlockTitle, uploader.BlockDescription, false);
